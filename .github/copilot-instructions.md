...
## Overview
- Purpose: AI English-learning assistant with chat, suggestions, and history.
...
- Frontend: [src/react/App.jsx](../src/react/App.jsx), [src/react/main.jsx](../src/react/main.jsx), [src/react/components/Chat.jsx](../src/react/components/Chat.jsx), [src/react/components/ChatApp.jsx](../src/react/components/ChatApp.jsx), [src/react/components/Login.jsx](../src/react/components/Login.jsx), [src/react/components/Sidebar.jsx](../src/react/components/Sidebar.jsx)
...
- **[src/react/components/ChatApp.jsx](../src/react/components/ChatApp.jsx)**: Main authenticated chat UI. Loads user chat history and chat list from backend using Supabase token. Handles sending messages, starting new chats, switching chats, and sign-out. Manages sidebar open/close state for mobile. Shows loading state while fetching data. Passes chat data to [Chat](../src/react/components/Chat.jsx).

- **[src/react/components/Sidebar.jsx](../src/react/components/Sidebar.jsx)**: Sidebar for chat navigation and user actions. Lists all user chats (with titles), allows starting a new chat, selecting a chat, toggling dark mode (with persistence), and logging out. Handles mobile sidebar open/close and account actions dropdown.

- **Chat rendering**: [Chat.jsx](../src/react/components/Chat.jsx) uses `marked` + `DOMPurify` for markdown rendering; code blocks render in `<pre><code>` style; all HTML is sanitized. Assistant messages are shown as `llm` or `assistant`. A welcome message is prepended to empty histories.
