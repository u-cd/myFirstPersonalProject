...
app.get('/chat-history', async (req, res) => {
    ...
    if (!chatId || !userId) return res.status(400).json({ error: 'Invalid request' });
...
app.get('/chats-with-title', async (req, res) => {
    ...
    if (!(isObjectId(userId) || isUUID(userId))) return res.status(400).json({ error: 'Invalid request' });
    try {
...
app.post('/writing-suggestions', async (req, res) => {
    ...
    try {
        ...
        // log for debugging (delete later)
...
app.post('/', async (req, res) => {
    ...
    if (!isNonEmptyString(userMessage, 2000)) return res.status(400).json({ error: 'Invalid request' });
    if (chatId && !isObjectId(String(chatId))) return res.status(400).json({ error: 'Invalid request' });
    if (userId && !(isObjectId(String(userId)) || isUUID(String(userId)))) return res.status(400).json({ error: 'Invalid request' });
    try {
