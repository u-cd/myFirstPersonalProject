...
なお、課金が発生する場合は、LLM（大規模言語モデル）の入出力に対してOpenAI API等の利用料がかかる部分のみを想定しています。
...
メインの機能としては、テストのような形式ではなく、自然な会話を通じて英語力が身につくアプリを目指しています。「勉強している」という意識がなるべくないよう、チャットボット形式で、ユーザーが友達と話しているような感覚を重視しています。さらに、ユーザーが積極的に英語でライティングを行えるような設計にしています。ライティングは英語学習において特に効果が高く、実際に自分の言葉で表現することで理解が深まり、英語力の向上につながります。
